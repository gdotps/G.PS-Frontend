...
import { ChevronLeft, Bookmark, Clock, MapPin, UserCheck, Send } from 'lucide-react';
  ...
  return (
    <div className="bg-white min-h-screen pb-32">
       <Header 
         ...
         onLeftClick={onBack}
         rightIcon={<Bookmark className={isBookmarked ? "text-gps-500 fill-gps-500" : "text-gray-400"} />}
       ...
       <div className="pt-16 px-5">
          {(post.images && post.images.length > 0) ? (
             <div className="flex gap-2 overflow-x-auto hide-scrollbar mb-6 -mx-5 px-5 snap-x">
                {post.images.map((img, i) => (
                    <div key={i} className="w-full h-64 flex-shrink-0 rounded-2xl overflow-hidden shadow-md snap-center relative">
          ...
          <div className="flex items-center gap-3 mb-6">
             <img src={post.authorAvatar} className="w-12 h-12 rounded-full border border-gray-100" />
             <div>
                <h3 className="text-gray-900 font-bold">{post.authorName}</h3>
                <p className="text-xs text-gray-500 bg-gray-100 inline-block px-1.5 py-0.5 rounded font-bold">호스트</p>
             </div>
          </div>
          ...
          <div className="border-t border-gray-100 pt-6 mt-6">
            <h3 className="font-bold text-gray-900 mb-4 px-1">댓글 ({post.comments ? post.comments.length : 0})</h3>
                ...
                {(!post.comments || post.comments.length === 0) ? (
                    ...
                    post.comments.map(comment => (
                        <div key={comment.id} className="flex gap-3">
                            <img src={comment.authorAvatar || `https://ui-avatars.com/api/?name=${comment.authorName}`} className="w-8 h-8 rounded-full bg-gray-200 object-cover border border-gray-50" alt="avatar" />
                            <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded-r-xl rounded-bl-xl inline-block">{comment.text}</p>
                            </div>
                        </div>
                    ))
                )}
            ...
            <form onSubmit={handleSubmit} className="flex gap-2 items-center">
                <div className="w-8 h-8 rounded-full bg-gray-200 shrink-0 overflow-hidden">
                   <img src={CURRENT_USER.avatarUrl} alt="me" className="w-full h-full object-cover" />
                </div>
       ...
       <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100 max-w-md mx-auto z-20">
          ...
          >
             {isHost ? '내가 만든 모임입니다' : hasApplied ? '참여 취소하기' : `참여하기 (프로필 전송)`}
          </button>
       </div>
    </div>
  );
